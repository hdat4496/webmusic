...
(function($)
{
	$(document).ready(function()
	{
		...
		$list_action.find('#submit').click(function(){ //tim toi the co id = submit,su kien click
			...
			$.ajax({
				...
				success: function()
				{
					...
					var num_del=ids.length;
					var total=$("#total").text()-num_del;
					$('#total').text(total);
		...
		// thêm nhiều bài hát
		var $list_action 	= $('.list_action');//tim toi the co class = list_action
		$list_action.find('#submit_ver2').click(function(){ //tim toi the co id = submit,su kien click
			if(!confirm('Bạn chắc chắn muốn thêm tất cả dữ liệu ?'))
			{
				return false;
			}
